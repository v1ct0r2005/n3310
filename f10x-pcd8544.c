...
unsigned int adr;
unsigned char data;
unsigned char lcd_YP;
unsigned char lcd_YC;
unsigned char x;
//==============
...
//	  GPIO_InitTypeDef GPIO_InitStructure;
	  ...
	  lcd8544_senddata(12);	    // 0b1100 - normal mode
			                        // 0b1101 - invert mode
...
void lcd8544_putchar(unsigned char px, unsigned char py, unsigned char ch, unsigned char mode) {
    ...
    lcd_YP=7- (py & 0x07);    // битовая позиция символа в байте
    lcd_YC=(py & 0xF8)>>3; 	// байтовая позиция символа на экране
